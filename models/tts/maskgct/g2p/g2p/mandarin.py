...
resource_path=os.sep.join(resource_path.split(os.sep)[:-1])
...
pinyin_file_path = os.path.join(resource_path, "sources/chinese_lexicon.txt")
word_pinyin_dict = {}
with open(pinyin_file_path, "r", encoding="utf-8") as fread:
...
pinyin_file_path = os.path.join(resource_path, "sources/pinyin_2_bpmf.txt")
...
pinyin_file_path = os.path.join(resource_path, "sources/bpmf_2_pinyin.txt")
bopomofos2pinyin_dict = {}
with open(
    pinyin_file_path, "r", encoding="utf-8"
) as fread:
    txt_list = fread.readlines()
    for txt in txt_list:
        v, k = txt.strip().split("\t")
