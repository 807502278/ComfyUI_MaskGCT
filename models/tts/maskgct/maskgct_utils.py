...
from models.utils.util import load_config
...
from models.load_model import load_model_list, OBJECT_DIR
...
def build_semantic_model(device):
    ...
    file_dir = os.path.join(
        OBJECT_DIR, "models/tts/maskgct/ckpt/wav2vec2bert_stats.pt")
    stat_mean_var = torch.load(file_dir)
...
class MaskGCT_Inference_Pipeline:
    ...
    def maskgct_inference(
        ...
        acoustic_code = self.extract_acoustic_code(
            torch.tensor(audio_2[1]).unsqueeze(0).to(self.device)
        )
